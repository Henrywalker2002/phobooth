...
function Profile() {
  ...
  const handleCheckAccount = (e) => {
    e.preventDefault();
    if (newInfo.account_number && (newInfo.account_number.length < 6 || newInfo.account_number.length > 19)) {
      setErrMsg({... errMsg,  account_name: "Tên chủ tài khoản không hợp lệ" });
      return;
    }
    if (newInfo.account_bin === "" && newInfo.account_bin === null && studioInfo.bank_bin === "" && studioInfo.bank_bin === null) {
    ...
    console.log(data);
    axios.post('https://api.vietqr.io/v2/lookup', data = data, {
      "headers" : {
      ...
      >
        ...
        <form onSubmit={handleUpdateStudio}>
          ...
          <div className="flex flex-col ml-4 mr-10">
            <div className="flex justify-between w-full">
              <div className="flex flex-col">
                ...
                <TextField
                  variant="outlined"
                  name="account_number"
                  value = {newInfo.account_number ?? studioInfo.account_number ?? ""}
                  onChange={updateStudioInfo}
                  sx={{
                    "& .MuiInputBase-input": {
                      height: "45px",
            ...
            <Button variant="outlined" onClick={handleCheckAccount} sx={{ width: "fit-content" }}> Kiểm tra </Button>
          ...
          <div className="flex  gap-5 ml-4 my-5 self-start">
            ...
            <Button
              ...
              disabled = {(newInfo.account_name === "" || newInfo.account_name === null) && (newInfo.account_number) ? true: false}
            >
              Lưu thông tin
            </Button>
          </div>
